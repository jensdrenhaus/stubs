...
/* Hier muesst ihr selbst Code vervollstaendigen */ 
	...
	if(!(status & 0x01))
	   return invalid;
	int data = data_port.inb();
	//s << "status: "<<hex<<status<<" "<<data<<"  "<<ctrl_port.inb()<<"\n";
	if((status & 0x20))
	{
		//s << "mouse\n";
		return invalid;
...
void Keyboard_Controller::set_repeat_rate (int speed, int delay)
 {
	...
	wait_status(0x02, false);
	
	data_port.outb(0xf3);
	
	wait_status(0x01, true);
	int data1 = data_port.inb();
	//s << data1 << "\n";
	if(data1 != 0xfa)
	{
		s << "no ack" << "\n";
	...
	int outdata = 0;
	if(speed <= 5) outdata = 0x14;
	else if(speed <= 7) outdata = 0x10;
	else if(speed <= 10) outdata = 0x0c;
	else if(speed <= 15) outdata = 0x08;
	else if(speed <= 20) outdata = 0x04;
	else if(speed <= 25) outdata = 0x02;
	else outdata = 0x00;
	outdata |= (delay << 5);
	//s << "outdata: "<<hex<<outdata << "\n";
	
	
	data_port.outb(outdata);
...
int Keyboard_Controller::read_data()
{
	wait_status(0x01, true);
...
void Keyboard_Controller::wait_status(int mask, bool on)
{
	while(true)
	{
		int status = ctrl_port.inb();
...
bool Keyboard_Controller::isPressed(unsigned char key)
{
	return keys[key];
}
